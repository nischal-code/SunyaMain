          ...
          <li key={comment._id} className="flex items-start gap-2.5">
            ...
            <div className="min-w-0 flex-1 rounded-xl bg-slate-50 px-3.5 py-2.5">
              <div className="flex items-baseline justify-between gap-2">
                <p className="truncate text-sm font-medium text-slate-900">{comment.user?.name || "Unknown"}</p>
                <p className="shrink-0 text-[11px] text-slate-400">{formatDateTime(comment.createdAt)}</p>
              </div>
              <p className="mt-0.5 whitespace-pre-wrap break-words text-sm text-slate-600">{comment.text}</p>
            </div>
          </li>
        ))}

        {!comments.length && <p className="py-2 text-sm text-slate-400">No comments yet.</p>}
      </ul>
