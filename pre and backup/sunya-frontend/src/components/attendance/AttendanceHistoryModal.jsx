...
const AttendanceHistoryModal = ({ isOpen, onClose, record, onEdit }) => {
  return (
    ...
    >
      {!record && <p className="py-6 text-center text-gray-400">No attendance recorded for this day.</p>}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {record.isManualEntry && (
              <span className="text-xs font-medium text-gray-400">Manual entry</span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 rounded-xl bg-gray-50 p-4">
            <div>
              ...
              <p className="mt-0.5 font-medium text-gray-900">{formatTime(record.checkIn)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-400">Check Out</p>
              <p className="mt-0.5 font-medium text-gray-900">{formatTime(record.checkOut)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-gray-400">Working Hours</p>
