...
const ProjectsPage = () => {
  ...
  // Re-runs each time a fresh page of projects lands, so cards/rows fade
  // and stagger in whenever the list changes (filters, pagination, view).
  const listRef = useGsapReveal({
    stagger: 0.05,
    y: 16,
    selector: view === "card" ? ":scope .grid > *" : ":scope tbody tr",
    deps: [projects, view, isLoading],
  });
