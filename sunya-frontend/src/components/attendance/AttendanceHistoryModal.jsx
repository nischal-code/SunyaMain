 ...
 *  - onClose:  fn — required
 *  - record:   { user?, date, clockIn, clockOut, totalHours, status,
 *      notes, isManualEntry } — the record to display; null renders
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100/60 p-4">
            <div>
              ...
              <p className="mt-0.5 font-semibold text-slate-900">{formatTime(record.clockIn)}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Check Out</p>
              <p className="mt-0.5 font-semibold text-slate-900">{formatTime(record.clockOut)}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400">Working Hours</p>
              <p className="mt-0.5 font-semibold text-slate-900">
                {record.totalHours !== undefined ? `${Number(record.totalHours).toFixed(1)}h` : "—"}
              </p>
            </div>
          </div>
