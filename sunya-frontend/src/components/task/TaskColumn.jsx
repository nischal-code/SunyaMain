...
const TaskColumn = ({
  ...
  return (
    <div
      ...
      className={`flex w-72 shrink-0 flex-col rounded-2xl border bg-gray-50/60 p-3 transition-colors ${
        isDragOver ? "border-primary-400 bg-primary-50/50" : "border-gray-200"
      } ${className}`}
    >
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-y-auto">
        {isLoading &&
          Array.from({ length: 2 }).map((_, index) => (
            <div key={index} className="h-32 w-full animate-pulse rounded-2xl bg-gray-100" />
          ))}
