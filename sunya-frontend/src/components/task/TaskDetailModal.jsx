...
const TaskDetailModal = ({ isOpen, onClose, task, currentUser, users = [], onTaskUpdate, onTaskDeleted }) => {
  ...
  return (
    <>
      ...
      >
        {actionError && (
          <div role="alert" className="mb-4 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-600">
            {actionError}
          </div>
        )}
        ...
        {isEditing ? (
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="ml-auto text-xs text-slate-400">Due {formatDate(task.dueDate)}</span>
            </div>

            {task.description && <p className="text-sm text-slate-600">{task.description}</p>}

            <div>
              <p className="mb-1.5 text-xs font-medium text-slate-500">Progress</p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {!task.assignedTo?.length && <p className="text-sm text-slate-400">Unassigned</p>}
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/60 px-3 py-3.5">
